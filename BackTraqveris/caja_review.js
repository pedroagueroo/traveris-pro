...
// 1. CONVERTIR MONEDA (L├│gica de Transacci├│n - Mantenemos tu c├│digo)
router.post('/convertir-moneda', async (req, res) => {
    ...
    try {
        ...
        if (!cotizacion) {
            ...
            if (!cotizacion) return res.status(500).send("No se pudo obtener la cotizaci├│n.");
        }
        ...
        res.json({ mensaje: "Conversi├│n realizada", cotizacion_utilizada: cotizacion, monto_convertido: monto_destino });
    } catch (err) {
...
// 3. OBTENER ├ÜLTIMOS 5 (Dashboard filtrado)
...
// 4. REGISTRAR PAGO Y ELIMINAR (Tus c├│digos originales)
